...
ClapTrap::ClapTrap( std::string const &name, unsigned const &hp, unsigned const &ep, int const &atk ) : Name(name),  HitPoints(hp), MaxHP(hp), EnergyPoints(ep), MaxEP(ep), AttackDamage(atk) {
	std::cout << "***ClapTrap with name <\x1b[32m" << name << "\x1b[0m>";
	std::cout << " crawled out from underground***\n";
}
